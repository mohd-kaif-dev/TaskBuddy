    ...
    <div className="space-y-4">
      {tasks.length === 0 ? (
        <motion.div
          ...
          className="bg-gradient-to-br from-[#14213d] to-[#1a2b4d] rounded-xl p-8 text-center border border-[#fca311]/20 shadow-xl"
        >
          <div className="flex flex-col items-center gap-6 ">
            ...
            >
              <div className="absolute -top-3 -right-3 animate-ping">
                <div className="w-4 h-4 bg-[#fca311] rounded-full opacity-75"></div>
              </div>
              <div className="w-20 h-20 bg-[#fca311]/10 rounded-full flex items-center justify-center">
                <FaPlus className="text-[#fca311] text-3xl animate-pulse" />
              </div>
            </div>

            <div className="space-y-3">
              <h3 className="text-2xl font-bold bg-gradient-to-r from-[#fca311] to-[#ffd700] bg-clip-text text-transparent">
              ...
              <div className="flex items-start gap-4 w-full">
                ...
                <div className="flex-grow">
                  <div className="flex flex-col md:flex-row items-start justify-between">
                    <h2 className="font-medium text-lg md:text-2xl pr-2">
                      {task.title}
                    </h2>
                    <div className="flex mt-1 md:mt-2 gap-4 whitespace-nowrap">
                      <span
                        className={`text-sm  ${
                          isTaskOverdue(task) ? "text-red-400" : "text-white/60"
                        }`}
                      >
                  ...
                  {task.milestones.length > 0 && (
                    <div className="mt-3 pl-2  border-l-2 border-white/10 space-y-2">
                      {task.milestones.map((milestone) => (
                        <div
                          key={milestone.id}
                          className="flex items-start gap-2 text-sm text-white/60 w-full"
                        >
                          <button
                            onClick={() => {
                              if (isTaskOverdue(task)) {
                          ...
                          <span
                            className={`text-sm md:text-lg items-start ${
              ...
              {editingTask === task.id && showAddMilestone && (
                <div className="mt-3 pl-8 flex items-center gap-2">
                  <input
                    ...
                    className="w-full bg-black/20 text-white border border-[#fca311]/20 rounded px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-[#fca311]/20"
                    onKeyPress={(e) => {
                      if (e.key === "Enter") {
